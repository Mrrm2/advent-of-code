...
#
# Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green,
# 1 blue Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red Game 4: 1 green, 3 red,
# 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green In game 1,
# three sets of cubes are revealed from the bag (and then put back again). The first set is 3 blue cubes and 4 red
...
def clean_data(text_array: list[str]):
    data = {}
    for number, game in enumerate(text_array, start=1):
        game = ((game.replace(f'Game {number}: ', '')
                .replace(';',  ''))
                .replace(',', '').split())
...
def easy(games_dict: dict):
    ...
    for game_num, games in games_dict.items():
        ...
        valid = True
        while j < len(games):
            if int(games[i]) > max_balls[games[j]]:
                valid = False
                break
...
def hard(games_dict: dict):
    total = 0
    for game_num, games in games_dict.items():
        i, j = 0, 1
        max_balls = {}
        while j < len(games):
            if games[j] in max_balls:
                max_balls[games[j]] = max(int(games[i]), max_balls[games[j]])
            else:
                max_balls[games[j]] = int(games[i])

            i += 2
            j += 2

        total += reduce(operator.mul, max_balls.values(), 1)
